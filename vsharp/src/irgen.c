...
static KType vs_get(const char *n) {
    for (VSym *v = vs_head; v; v = v->next)
        if (!strcmp(v->name, n)) return v->ktype;
    return KT_I32;                     /* default int */
}

/* --- id → tamanho de array literal --------------------------------*/
...
static int arr_len(const char *id) {
    for (ArrInfo *a = arr_head; a; a = a->next)
        if (!strcmp(a->name, id)) return a->len;
    return 0;
}

/*======================================================================
  REGISTRADORES, RÓTULOS, CONSTANTES GLOBAIS
  --------------------------------------------------------------------*/
static FILE *out;
...
static char *new_lbl(const char *p) {
    char *b = malloc(32);
    sprintf(b, "%s%d", p, lblc++);
    return b;
}

static char *emit_num(long v) {
    char *s = malloc(32);
    sprintf(s, "%ld", v);
    return s;
}

static char *emit_float(double d) {
    char *s = malloc(64);
    sprintf(s, "%f", d);
    return s;
}
...
static void add_gconst(char *s) {
    if (gcnt == gcap) {
        gcap = gcap ? gcap * 2 : 16;
        gconsts = realloc(gconsts, gcap * sizeof(GConst));
    }
    gconsts[gcnt++].def = s;
}
...
static char *emit_cstr(const char *lex)
{
    ...
    sprintf(line,
            "@.str%d = private constant [%d x i8] c\"%s\\0D\\0A\\00\"\n",
            id, len, esc);
    add_gconst(line);

    char *ptr = new_reg();
    fprintf(out,
        "  %s = getelementptr inbounds [%d x i8], [%d x i8]* @.str%d, "
        "i32 0, i32 0\n",
        ptr, len, len, id);
    return ptr;
}

static char *emit_cstr_raw(const char *lex)
{
    char esc[512]; int j = 0;
    for (int i = 0; lex[i] && j < 510; i++) {
        if (lex[i] == '\\' || lex[i] == '"') esc[j++] = '\\';
    ...
    int len = j + 1;                     /* +NUL apenas */
    int id  = strc++;

    char *line = malloc(600);
    sprintf(line,
            "@.sraw%d = private constant [%d x i8] c\"%s\\00\"\n",
            id, len, esc);
    add_gconst(line);
    ...
    fprintf(out,
        ...
        "[%d x i8], [%d x i8]* @.sraw%d, i32 0, i32 0\n",
        ptr, len, len, id);
    return ptr;
}
...
{
    int len = (int)strlen(spec) + 3;        /* \r + \n + NUL */
    int id  = strc++;

    char *line = malloc(120);
    sprintf(line,
            "@.fmt%d = private constant [%d x i8] c\"%s\\0D\\0A\\00\"\n",
            id, len, spec);
    add_gconst(line);

    char *ptr = new_reg();
    fprintf(out,
        "  %s = getelementptr inbounds [%d x i8], [%d x i8]* @.fmt%d, "
        "i32 0, i32 0\n",
        ptr, len, len, id);
    return ptr;
}
...
static char *emit_array(Node *n) {
    ...
    for (int i = 0; i < cnt; i++)
        elems[i] = emit_expr(n->v.array.elems[i], &kinds[i]);
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        fprintf(out,
            ...
            gep, buf, i);
        fprintf(out,
            "  store double %s, double* %s\n",
            val, gep);
        free(val); free(gep);
    }
    free(elems); free(kinds);
    return buf;                            /* ponteiro double* */
}
...
/*======================================================================
  ...
  --------------------------------------------------------------------*/
/*==================================================================*/
/*  PRINT (...): últimos sem espaço, \r\n vem do próprio formato    */
/*==================================================================*/
static void emit_print(Node *p)
{
    for (int i = 0; i < p->v.print.count; i++) {
        ...
        if (it->type == NODE_STR) {
            int last = (i + 1 == p->v.print.count);

            /* escolhe literal com ou sem CRLF */
            char *ptr = last ? emit_cstr(it->v.str)      /* com \r\n */
                            : emit_cstr_raw(it->v.str); /* sem \r\n */

            if (last) {
                /* imprime direto, já tem newline */
                fprintf(out,
                    "  call i32 (i8*, ...) @printf(i8* %s)\n", ptr);
            } else {
                /* ainda vem mais coisa: "%s " sem newline */
                char *pf = emit_format("%s ");
                fprintf(out,
                    "  call i32 (i8*, ...) @printf(i8* %s, i8* %s)\n",
                    pf, ptr);
                free(pf);
            }
            free(ptr);
            ...
            const char *fmt = (i + 1 < p->v.print.count)
                              ? (k ? "%f " : "%d ")
                              : (k ? "%f"  : "%d");

            char *pf = emit_format(fmt);
            fprintf(out,
    ...
    /* nada de emit_cstr("") aqui – o \r\n já saiu no último formato */
}

/*======================================================================
  CHAMADAS BUILT-IN  (npv / irr / wacc)
  --------------------------------------------------------------------*/
static char *emit_call(Node *n, int *kind)
{
    ...
    if (!strcmp(fn, "npv")) {
        int k0, k1;
        char *rate = emit_expr(n->v.call.args[0], &k0);
        ...
        char *r = new_reg();
        fprintf(out,
            "  %s = call double @npv(double %s, double* %s, i32 %s)\n",
            r, rate, arr, cnt);
    ...
    if (!strcmp(fn, "irr")) {
        ...
        char *r = new_reg();
        fprintf(out,
            "  %s = call double @irr(double* %s, i32 %s)\n",
            r, arr, cnt);
        free(arr); free(cnt);
        *kind = 1; return r;
    }
    ...
    if (!strcmp(fn, "wacc")) {
        int kE, kD, kKe, kKd;
        char *E  = emit_expr(n->v.call.args[0], &kE);
        char *D  = emit_expr(n->v.call.args[1], &kD);
        char *Ke = emit_expr(n->v.call.args[2], &kKe);
        char *Kd = emit_expr(n->v.call.args[3], &kKd);

        /* int → double */
        if (kE  == 0) { char *c = new_reg(); fprintf(out,"  %s = sitofp i32 %s to double\n",c,E);  free(E);  E  = c; }
        if (kD  == 0) { char *c = new_reg(); fprintf(out,"  %s = sitofp i32 %s to double\n",c,D);  free(D);  D  = c; }
        if (kKe == 0) { char *c = new_reg(); fprintf(out,"  %s = sitofp i32 %s to double\n",c,Ke); free(Ke); Ke = c; }
        if (kKd == 0) { char *c = new_reg(); fprintf(out,"  %s = sitofp i32 %s to double\n",c,Kd); free(Kd); Kd = c; }

        char *r = new_reg();
        fprintf(out,
            "  %s = call double @wacc(double %s, double %s, double %s, double %s)\n",
            r, E, D, Ke, Kd);
        free(E); free(D); free(Ke); free(Kd);
        *kind = 1; return r;
    }

    /* fallback */
    *kind = 1;
    return emit_float(0.0);
}
...
static char *emit_expr(Node *n, int *kind)
{
    ...
    case NODE_VARDECL: {
        ...
        char *val = NULL;
        if (n->v.vdecl.init)
            val = emit_expr(n->v.vdecl.init, &k);

        const char *ty = (k == 0) ? "i32" :
                         (k == 1) ? "double" : "double*";
        fprintf(out,
            "  %%%s = alloca %s\n", n->v.vdecl.id, ty);
        vs_add(n->v.vdecl.id,
               (k == 0) ? KT_I32 : (k == 1) ? KT_DBL : KT_PTR);

        if (val) {
            /* int → double se destino double */
            if (k == 0 && ty[0] == 'd') {
                char *c = new_reg();
                fprintf(out,
                    "  %s = sitofp i32 %s to double\n", c, val);
            ...
            fprintf(out,
                "  store %s %s, %s* %%%s\n",
                ty, val, ty, n->v.vdecl.id);
            if (n->v.vdecl.init->type == NODE_ARRAY)
                arr_add(n->v.vdecl.id,
                        n->v.vdecl.init->v.array.count);
            free(val);
        }
    ...
    case NODE_ASSIGN: {
        ...
        if (n->v.assign.expr->type == NODE_ARRAY)
            arr_add(n->v.assign.id,
                    n->v.assign.expr->v.array.count);
        free(val);
        *kind = 0; return emit_num(0);
    }
    ...
    /*-------------------------------------------------- pair ----------*/
    case NODE_PAIR:           /* nunca avaliado sozinho */
        *kind = 0; return emit_num(0);
    ...
    /* não implementados / NOP ----------------------------------------*/
    case NODE_FOR:    *kind = 0; return emit_num(0);
...
{
    char *Lc = new_lbl("while_cond"),
         *Lb = new_lbl("while_body"),
         *Le = new_lbl("while_end");
...
static char *emit_bin(Node *n, int *kind)
{
    ...
    /* promoção int → double quando necessário */
    if (kl != kr) {
        if (kl == 0) {
            char *c = new_reg();
            fprintf(out,
...
{
    /* 1. Emite corpo de main para tmp */
    FILE *tmp = tmpfile();
    if (!tmp) { perror("tmpfile"); exit(1); }
    out = tmp;
    int k; char *dummy = emit_expr(root, &k);
    free(dummy);
    ...
    for (int i = 0; i < gcnt; i++)
        fputs(gconsts[i].def, f);
    if (gcnt) fputc('\n', f);

    /* 5. Função main */
    fputs("define i32 @main(){\n", f);
    int ch;
    while ((ch = fgetc(tmp)) != EOF) fputc(ch, f);
    fputs("  ret i32 0\n}\n", f);
